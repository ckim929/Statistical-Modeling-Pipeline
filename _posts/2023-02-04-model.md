...
This step is where the "learning" will begin. The model built using the training data will then be used to predict the output value. Over several iterations, the model will be improved for optimal prediction accuracy. The model's performance will be evaluated against the test data using the appropriate metrics. 
